...
vis_plot_hist <- function(wl_hist, target = 18, percentage = 92) {
    ...
    ggplot2::ggplot(wl_hist_, ggplot2::aes(x = .data$weeks_waiting)) +
        ...
        ggplot2::annotate("text", x = cutoff_2, y = max(wl_hist_$n), label = "52 weeks ago",
                angle = 90, vjust = -0.4, hjust = 1, color = "red") +
        ggplot2::annotate("text", x = cutoff_1, y = max(wl_hist_$n), label = "18 weeks ago",
                angle = 90, vjust = -0.4, hjust = 1, color = "blue") +
        ggplot2::annotate("text", x = percentile$weeks, y = max(wl_hist_$n), label = "92nd percentile",
