...
If we divide that 840 on the waiting list by 20, we get 42 1weeks.  
...
If we are governing our resources well, we want to allow a sustainable queue as this allows patients to be flexibly scheduled whilst still meeting our targets.  All capacity comes at a cost to the system, so we are seeking a balance between demand and capacity with a sustainable waiting list.
...
ggplot(combine_queue, aes(y = queue_size, x = dates)) +
  ...
  labs(y = "Queue Size", x = "Date"
       , title = "Simulation reduction in waiting list using demand or capacity")
...
We, again, have the same target of 92% within 18 weeks (as per example 1).  This means our target average waiting time would be `r round(target_mean_wait, 2)`. 
...
We can then verify this using simulation, following the steps in example 1.: set up the waiting list, then simulate with our parameters, then plot:
...
ggplot(queue_over_time, aes(y = queue_size, x = dates)) +
  ...
  annotate("text", label = "Target queue size", x = ctrl_tbl$start_date[5]
           , y = ctrl_tbl$target_queue_size[5], vjust = 1.2, hjust = 0.2, colour = "#A69D75FF") +
