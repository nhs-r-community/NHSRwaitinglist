...
```{r dump_wl_setup}
current_wl1 <- data.frame(Referral = rep(as.Date("2023-03-31"), 840) 
                         , Removal = rep(as.Date(NA), 840)
    )
# There is only a single day in our waiting list so we don't need `tail()` this time
...
```{r target1}
target_queue1 <- calc_target_queue_size(demand = 100, target_wait = 18, factor = qt92)
target_queue1
```
...
ggplot(raised_capacity_queue, aes(y = queue_size, x = dates)) +
  ...
  annotate("text", label = "Target queue size", x = as.Date("2024-03-31")
           , y = target_queue1, vjust = 1.2, hjust = 1.2, colour = "#440154FF") +
...
```{r target2}
target_queue2 <- calc_target_queue_size(demand = 50, target_wait = 18, factor = qt92)
target_queue2
```
...
ggplot(raised_wl, aes(y = queue_size, x = dates)) +
  ...
  labs(y = "Queue Size", x = "Date", title = "Simulated waiting list after allowing waiting list to grow")
```


# 3. How our list will change with predicted population/referral growth

This more complex example will make two assumptions a 5-year modelling period:

1. That referrals will grow 5% per year
2. In year 3, a service change will come into place that removes 20% of the referrals.

We will set up a scenario starting with 100 referrals per week and 100 capacity, and a waiting list of 500.
We will do our range calculations for the sustainable lists, assuming different target from the first 2 examples, where 95% should be seen within 18 weeks.
...
target_queue3 <- calc_target_queue_size(demand = 100, target_wait = 18, factor = qexp(0.95))
target_queue3

```
In this case ours sustainable waiting list is `r target_queue3`, but this is only true at the start of the period. Given our growth and change assumptions, we need to construct a control table that works these values out.  This is worked through in R, but may be easier for users in a spreadsheet application.

```{r control_table}

ctrl_tbl <- data.frame(
  start_date = as.Date(c("2024-04-01","2025-04-01","2026-04-01","2027-04-01","2028-04-01"), format = "%Y-%m-%d"),
  end_date = as.Date(c("2025-03-31","2026-03-31","2027-03-31","2028-03-31","2029-03-31"), format = "%Y-%m-%d"),
  demand = 100,
  capacity = 100
)
...
ctrl_tbl$target_queue_size <- calc_target_queue_size(demand = ctrl_tbl$demand, target_wait = 18, factor = qexp(0.95))
```

```{r setup_wl_3}
current_wl3 <- data.frame(Referral = rep(as.Date("2024-03-31"), 500) 
...
mc_agg<- aggregate(queue_size~dates, data=mc_bind , FUN = \(x) 
              c(mean_q = mean(x),
                median_q = median(x), 
...
Now we will produce a similar plot to the one in section 1, but using the average value and 95% confidence interval.

```{r montecarlo_3}

ggplot(mc_agg, aes(y = mean_q, x = dates)) +
  geom_line() +
  geom_ribbon(aes(ymin=lower_95CI, ymax=upper_95CI)#, colour = "#440154FF"
              #, fill = "#440154FF"
