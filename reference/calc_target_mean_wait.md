...
and a numerical value for factor. The average wait is actually the
target mean wait and is calculated as follows: target_wait / factor. If
we want to have a chance between 1.8%-0.2% of making a waiting time
target, then the average patient should have a waiting time between a
quarter and a sixth of the target. Therefore: The mean wait should sit
somewhere between target_wait/factor=6 \< Average Waiting Time \<
target_wait/factor=4.
...
- factor:
  ...
  the target use factor=4 and one sixth of the target use factor = 6
  etc. Defaults to 4.
