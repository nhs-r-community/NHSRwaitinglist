test_that("wl_referral_stats errors with incorrect arg classes", {
  ...
  expect_error(wl_referral_stats(waiting_list, start_date = data.frame()), start_msg)
  ...
  expect_error(wl_referral_stats(waiting_list, end_date = data.frame()), end_msg)

  ref_msg <- "`referral_index` must be of class <numeric/character/logical>"

  expect_error(wl_referral_stats(waiting_list, referral_index = list()), ref_msg)
