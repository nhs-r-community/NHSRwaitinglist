...
test_that("wl_schedule throws error for invalid schedule input", {
  ...
  expect_error(
    wl_schedule(waiting_list, invalid_schedule),
...
test_that("wl_schedule errors with incorrect arg classes", {
  ...
  expect_error(
    wl_schedule(waiting_list, schedule, referral_index = list()),
    ref_msg
  )
  expect_error(
    wl_schedule(waiting_list, schedule, referral_index = NULL),
    ref_msg
  )
  expect_error(
    wl_schedule(waiting_list, schedule, referral_index = sum),
    ref_msg
  )
  ...
  expect_error(
    wl_schedule(waiting_list, schedule, removal_index = list()),
    rem_msg
  )
  expect_error(
    wl_schedule(waiting_list, schedule, removal_index = NULL),
    rem_msg
  )
  expect_error(
    wl_schedule(waiting_list, schedule, removal_index = sum),
    rem_msg
  )
  ...
  expect_error(
    wl_schedule(waiting_list, schedule, unscheduled = list()),
    unsch_msg
  )
  expect_error(
    wl_schedule(waiting_list, schedule, unscheduled = NULL),
    unsch_msg
  )
  expect_error(
    wl_schedule(waiting_list, schedule, unscheduled = sum),
    unsch_msg
  )
})
...
test_that("wl_schedule errors for invalid indexes to `waiting_list", {
  ...
  expect_error(
    wl_schedule(iris, schedule, "bad index"),
    r"(`referral_index` with value "bad index")"
  )
  expect_error(
    wl_schedule(iris, schedule, referral_index = 10),
  ...
  expect_error(
    wl_schedule(iris, schedule, removal_index = "bad index"),
    not_found
  )
  ...
  expect_error(
    wl_schedule(iris, schedule, removal_index = 10),
